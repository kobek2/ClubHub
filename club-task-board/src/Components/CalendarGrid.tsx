...
const CalendarGrid: React.FC<CalendarGridProps> = ({ events, tasks, onEventClick }) => {
  ...
  const dataByDay: Record<string, { events: Event[]; tasks: Task[] }> = {};
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const overdueTasks = dayData.tasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && t.status !== 'DONE').length;
